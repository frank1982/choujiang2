...
exports.main = async (event, context) => {
  ...
  //创建商品抽奖订单
  const createNewOrder = await db.collection('shopTakeTicketRecord').add({
  ...
  if (ticketCheck.data.length <= 0) {//没有该用户
    ...
    const shopAdd = await db.collection("shopTickets")//await 必须要加
    .add({
      data: {
    ...
    if (shopAdd.errMsg != 'collection.add:ok') {

      result["code"] = 6003 //创新新记录失败
      return result
    }else{
      result["code"] = 6666
      return result
